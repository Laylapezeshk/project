...
class TestTwoTeammatesOneMission(unittest.TestCase):
    ...
    def test_same_team_members_working_on_same_mission(self):

        for i in range(self.num_browsers):
            self.student_browsers[i].student_end_tour()
            time.sleep(6)

        self.student_browsers[0].student_do_mission()
        time.sleep(5)
        self.assertEqual(self.student_browsers[0].players_in_mission(), self.student_name[0],
                         "contributor name incorrect")
        self.student_browsers[1].student_do_mission()
        time.sleep(5)
        player_list = [self.student_name[0], self.student_name[1]]
        players_string = self.student_browsers[0].players_in_mission()
        p_string = players_string.replace(" ", "")
        p_list = p_string.split(',')
        self.assertEqual(p_list, player_list, 'name of the players in mission failed')
        self.student_browsers[0].student_do_quizzes()
        time.sleep(5)
        completed_mission = self.student_browsers[0].player_completed_mission()
        self.assertEqual(self.student_browsers[0].player_completed_mission(), self.student_name[0],
                         "completed player name failed")
        self.assertEqual(self.student_browsers[1].player_completed_mission(), self.student_name[0],
                         "completed player name failed")
